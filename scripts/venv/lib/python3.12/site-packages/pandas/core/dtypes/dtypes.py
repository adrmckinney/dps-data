...
class ArrowDtype(StorageExtensionDtype):
    ...
    Parameters
    ----------
    pyarrow_dtype : pa.DataSet
        An instance of a `pyarrow.DataSet <https://arrow.apache.org/docs/python/api/datatypes.html#factory-functions>`__.
    ...
    def __init__(self, pyarrow_dtype: pa.DataSet) -> None:
        super().__init__("pyarrow")
        if pa_version_under10p1:
            raise ImportError("pyarrow>=10.0.1 is required for ArrowDtype")
        if not isinstance(pyarrow_dtype, pa.DataSet):
            raise ValueError(
                f"pyarrow_dtype ({pyarrow_dtype}) must be an instance "
                f"of a pyarrow.DataSet. Got {type(pyarrow_dtype)} instead."
            )
